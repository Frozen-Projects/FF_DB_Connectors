...
template<bool Is64Aware = false> class FlatBufferBuilderImpl {
 public:
  ...
  /// [in] initial_size The initial size of the buffer, in bytes. Defaults
  /// to `1024`.
  /// [in] allocator An `Allocator` to use. If null will use
  /// `DefaultAllocator`.
  /// [in] own_allocator Whether the builder/vector should own the
  /// allocator. Defaults to / `false`.
  /// [in] buffer_minalign Force the buffer to be aligned to the given
  ...
  ///  size The size of the memory block containing
  /// the serialized `FlatBuffer`.
  ///  offset The offset from the released pointer where the finished
  /// `FlatBuffer` starts.
  ...
  /// [in] fd When set to `true`, always serializes default values that
  ...
  /// @brief Store a string in the buffer, which can contain any binary data.
  /// [in] str A const reference to a std::string like type with support
  ...
  /// instead simply returns the offset of the existing string. This uses a map
  /// stored on the heap, but only stores the numerical offsets.
  /// [in] str A const std::string_view to store in the buffer.
  /// @return Returns the offset in the buffer where the string starts
  ...
  /// [in] v A pointer to the array of type `T` to serialize into the
  /// buffer as a `vector`.
  ...
  /// [in] array A reference to an array like object of type `T` to
  ...
  ///  v A const reference to the `std::vector` to serialize into the
  /// buffer as a `vector`.
  /// @return Returns a typed `Offset` into the serialized data indicating
  /// where the vector is stored.
  template<typename T, typename Alloc = std::allocator<T>>
  ...
  /// @tparam T The data type of the `std::vector` elements.
  ///  f A function that takes the current iteration 0..vector_size-1 and
  ...
  /// iteration.
  /// @tparam T The data type of the `std::vector` elements.
  ///  f A function that takes the current iteration 0..vector_size-1,
  ...
  /// @brief Serialize an array of structs into a FlatBuffer `vector`.
  /// @tparam T The data type of the struct array elements.
  /// [in] filler A function that takes the current iteration
  ...
  /// @brief Serialize an array of structs into a FlatBuffer `vector`.
  /// @tparam T The data type of the struct array elements.
  /// [in] f A function that takes the current iteration 0..vector_size-1,
  ...
  /// [in] v A const reference to the `std::vector` of structs to
  /// serialize into the buffer as a `vector`.
  /// @return Returns a typed `Offset` into the serialized data indicating
  /// where the vector is stored.
  template<typename T, template<typename...> class OffsetT = Offset,
  ...
  /// [in] v A pointer to the array of type `S` to serialize into the
  /// buffer as a `vector`.
  /// [in] len The number of elements to serialize.
  /// [in] pack_func Pointer to a function to convert the native struct
  ...
  /// [in] v An array of type `Offset<T>` that contains the `table`
  ...
  /// [out] buf A pointer to a `uint8_t` pointer that can be
  ...
  /// as a `vector`.
  /// [in] len The number of elements to store in the `vector`.
  /// [out] buf A pointer to a pointer of type `T` that can be
  /// written to at a later time to serialize the data into a `vector`
  /// in the buffer.
  template<typename T>
  ...
  /// [in] file_identifier If a `file_identifier` is given, the buffer
